...
public class WGenerator extends JFrame implements ActionListener, FocusListener {
	...
	private JPanel pnlOreControls, pnlOreOptions, pnlOreButton;
	...
	public WGenerator(String title, int width, int height) {
		...
		pnlControls.setPreferredSize(new Dimension(440, Constants.WINDOW_SIZE));
		...
		pnlOreOptions.setLayout(new GridLayout(4,6,4,4));
		...
		cmbBiomeType.setSelectedIndex(12); // 12 = SAND
				...
				pnlOreOptions.add(lblTin);
				pnlOreOptions.add(txtTin);
				pnlOreOptions.add(lblMarble);
				pnlOreOptions.add(txtMarble);
				pnlOreOptions.add(lblSlate);
				pnlOreOptions.add(txtSlate);
				pnlOreOptions.add(lblAddy);
				pnlOreOptions.add(txtAddy);
				pnlOreOptions.add(lblGlimmer);
				pnlOreOptions.add(txtGlimmer);
				pnlOreOptions.add(lblRock);
				pnlOreOptions.add(txtRock);

				pnlOreButton.add(btnGenOres);

				pnlOreControls.add(pnlOreOptions, BorderLayout.CENTER);
				...
				loadingBar.setFont(loadingFont);
				...
				loadingBar.setBackground(new Color(250,100,100));
				loadingBar.setForeground(new Color(100,250,100));
				loadingBar.setEnabled(true);
				loadingBar.setValue(100);
				this.add(loadingBar, BorderLayout.NORTH);

				this.setBounds(0, 0, width + 440, height + 45);
	...
	public void newHeightMap(long seed, int mapSize, double resolution, int iterations, int minEdge, double borderWeight, int maxHeight, boolean moreLand) {
		...
		heightMap.generateHeights(loadingBar); 
	...
	private void updateMapView(boolean apiView, int viewType) {
		if (!apiView) {
			...
			for (int i = 0; i < heightMap.getMapSize(); i++) {
				loadingBar.setValue((int)((float)i/heightMap.getMapSize()*98f));
			...
			new Thread() {
				...
				public void run() {
					try {
						...
						JOptionPane.showMessageDialog(gen, "Error parsing number " + nfe.getMessage().toLowerCase(), "Error Generating HeightMap", JOptionPane.ERROR_MESSAGE);
					} finally {
						stopLoading();
					}
				}
				...
				public void run() {
					...
					try {
						heightMap.erode(Integer.parseInt(txtErodeIterations.getText()), Integer.parseInt(txtErodeMinSlope.getText()), 
								Integer.parseInt(txtErodeMaxSediment.getText()), loadingBar);

						updateMapView(false, 0);

						genHistory.add("ERODE:" + txtErodeIterations.getText() + "," + txtErodeMinSlope.getText() + "," + txtErodeMaxSediment.getText());
					} catch (NumberFormatException nfe) {
						JOptionPane.showMessageDialog(gen, "Error parsing number " + nfe.getMessage().toLowerCase(), "Error Eroding HeightMap", JOptionPane.ERROR_MESSAGE);
					} finally {
						stopLoading();
					}
				}
				...
				public void run() {
					...
					try {
						...
						tileMap.dropDirt(Integer.parseInt(txtDirtAmnt.getText()), Integer.parseInt(txtDirtSlope.getText()), 
								Integer.parseInt(txtDirtDiagSlope.getText()), Integer.parseInt(txtMaxDirtHeight.getText()), loadingBar);
						...
						JOptionPane.showMessageDialog(gen, "Error parsing number " + nfe.getMessage().toLowerCase(), "Error Dropping Dirt", JOptionPane.ERROR_MESSAGE);
					} finally {
						stopLoading();
					}
				}
				...
				public void run() {
					...
					try {
						...
						tileMap.plantBiome(Integer.parseInt(txtBiomeSeedCount.getText()), Integer.parseInt(txtBiomeSize.getText()), 
								...
								Integer.parseInt(txtBiomeMaxHeight.getText()), (Tile) cmbBiomeType.getSelectedItem(), loadingBar);
			...
			new Thread() {
				...
				public void run() {
					...
					try {
						...
						for (int i = 0; i < heightMap.getMapSize(); i++) {
							for (int j = 0; j < heightMap.getMapSize(); j++) {
								loadingBar.setValue((int)((float)(i*heightMap.getMapSize()+j)/(heightMap.getMapSize()*heightMap.getMapSize())*90f));
			...
			new Thread() {
				...
				public void run() {
					...
					try {
						...
						tileMap.generateOres(rates, loadingBar);
						...
						JOptionPane.showMessageDialog(gen, "Error parsing number " + nfe.getMessage().toLowerCase(), "Error Generating Ores", JOptionPane.ERROR_MESSAGE);
					} finally {
						stopLoading();
					}
				}
